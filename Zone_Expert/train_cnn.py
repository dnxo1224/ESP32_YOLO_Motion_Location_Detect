"""
Zone Expert Action Classifier — CNN 학습 스크립트

[사용법]
  ZONE_ID 변수를 0 / 1 / 2 / 3 으로 직접 바꾼 뒤 실행하면
  해당 Zone 전문가 CNN 모델을 학습한다.
...
[출력]
  weights_cnn/zone_expert_action_{ZONE_ID}_best.pt
  results/cnn_zone_expert_action_{ZONE_ID}_cm.png
"""
import os
import sys
import random
...
# 예) 둘 다 True → 'cnn' / 둘 다 False → 'cnn_norx_noos'
_tag = 'cnn'
...
def train(zone_id: int = ZONE_ID):
    ...
    oversample_note = f'Zone {zone_id} ×2 오버샘플링 적용' if USE_OVERSAMPLE else '오버샘플링 없음'
    print(f"  Train windows : {len(train_win)} ({oversample_note})")
    print(f"  Test  windows : {len(test_win)} (전체 Zone, 필터 없음)")

    # ── 모델 ─────────────────────────────────────────────────────────────────
    print(f"\n[4] Building ZoneExpertCNN (zone_id={zone_id}, use_rx_weight={USE_RX_WEIGHT})...")
    model = ZoneExpertCNN(zone_id=zone_id, use_rx_weight=USE_RX_WEIGHT).to(device)
    print(model)
    ...
    for epoch in tqdm(range(EPOCHS), desc=f'Zone{zone_id} Expert CNN'):
        model.train()
    ...
    print(f"\n[Best] Zone {zone_id} Expert CNN — Acc: {best_acc:.4f}  (epoch {best_epoch})")
    ...
    print(f"\nClassification Report (Zone {zone_id} Expert CNN, window-level, 전체 테스트):")
