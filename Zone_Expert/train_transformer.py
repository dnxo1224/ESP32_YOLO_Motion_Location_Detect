"""
Zone Expert Action Classifier — Transformer 학습 스크립트

[사용법]
  ZONE_ID 변수를 0 / 1 / 2 / 3 으로 직접 바꾼 뒤 실행하면
  해당 Zone 전문가 Transformer 모델을 학습한다.
  None으로 설정 시 Zone 0~3 전체를 순서대로 학습한다.

[평가 방식]
  윈도우 1개 → 즉시 예측 (실시간 추론 방식과 동일)

[출력]
  weights_transformer/zone_expert_action_{ZONE_ID}_best.pt
  results/transformer_zone_expert_action_{ZONE_ID}_cm.png
"""
import os
import sys
import numpy as np
...
BATCH_SIZE   = 64
EPOCHS       = 20
LR           = 5e-4      # Transformer는 LR 민감도가 높아 CNN(1e-3)보다 낮게 설정
WEIGHT_DECAY = 1e-4
GRAD_CLIP    = 1.0
...
def train(zone_id: int = ZONE_ID):
    ...
    for epoch in tqdm(range(EPOCHS), desc=f'Zone{zone_id} Expert Transformer'):
        model.train()
    ...
    print(f"\n[Best] Zone {zone_id} Expert Transformer — Acc: {best_acc:.4f}  (epoch {best_epoch})")
    ...
    print(f"\nClassification Report (Zone {zone_id} Expert Transformer, window-level, 전체 테스트):")
