"""Bi-LSTM — Zone Classification only (단일 태스크 비교군)"""
...
TEST_SUBJECTS = ['kms']
LABEL_NAMES   = ['Zone0', 'Zone1', 'Zone2', 'Zone3']
TASK          = 'zone'

PROCESSED_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'processed')
RESULTS_DIR   = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', 'lstm_zone')
...
class BiLSTMZone(nn.Module):
    def __init__(self):
        super().__init__()
        self.lstm = nn.LSTM(NUM_FEATURES, LSTM_HIDDEN, LSTM_LAYERS,
                            batch_first=True, bidirectional=True,
                            dropout=0.2 if LSTM_LAYERS > 1 else 0.0)
        self.fusion = nn.Sequential(
            nn.Linear(NUM_RX * LSTM_HIDDEN * 2, FUSION_DIM), nn.GELU(), nn.Dropout(0.3))
    ...
    def forward(self, x):
        B = x.size(0)
        seq = x.reshape(B * NUM_RX, L_INPUT, NUM_FEATURES)
        _, (h_n, _) = self.lstm(seq)
        h = torch.cat([h_n[-2], h_n[-1]], dim=-1).reshape(B, NUM_RX * LSTM_HIDDEN * 2)
        return self.head(self.fusion(h))
...
def benchmark_inference(model, loader, device, warmup_batches=3):
    ...
    print(f'\n{"="*60}')
    print(f'Inference Benchmark  [Bi-LSTM / Zone]')
    print(f'{"="*60}')
...
def save_cm(y_true, y_pred, acc):
    ...
    ax.set_title(f'Zone  (acc={acc*100:.1f}%)'); plt.tight_layout()
    p = os.path.join(RESULTS_DIR, 'zone_cm.png'); fig.savefig(p, dpi=120); plt.close(fig)
    print(f'  CM saved → {p}')
...
def plot_curves(history):
    ...
    axes[1].plot(ep, [a*100 for a in history['acc']], 'g-')
    axes[1].set(title='Zone Accuracy (%)', xlabel='Epoch'); axes[1].grid(alpha=0.3)
    plt.suptitle('Bi-LSTM / Zone', fontsize=13, y=1.02); plt.tight_layout()
...
def main():
    ...
    device = get_device()
    print(f'Model  : Bi-LSTM / Zone only  |  Device: {device}')
    ...
    weight_path = os.path.join(WEIGHTS_DIR, 'lstm_zone_best.pt')
    ...
    with open(os.path.join(RESULTS_DIR, 'summary.csv'), 'w') as f:
        ...
        f.write(f'BiLSTM,zone,{acc*100:.4f},{best_ep},'
                f'{bench["per_sample"]:.3f},{bench["n_samples"]/(bench["total_ms"]/1000):.1f}\n')

    print(f'\n{"="*60}\nRESULTS\n{"="*60}')
    print(f'  Zone Accuracy  : {acc*100:.2f}%')
