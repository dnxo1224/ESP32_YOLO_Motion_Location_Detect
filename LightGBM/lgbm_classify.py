...
def extract_action_features_window(window):
    """(WINDOW_SIZE, 166) → 1992-dim (action 특화: mean + std + FFT 저주파)"""
    mean = window.mean(axis=0)
    std  = window.std(axis=0)
    fft_power   = np.abs(np.fft.rfft(window, axis=0)) ** 2
    fft_lowfreq = fft_power[1:K_LOW + 1, :].flatten()    # (K_LOW×166,)
    return np.concatenate([mean, std, fft_lowfreq])
