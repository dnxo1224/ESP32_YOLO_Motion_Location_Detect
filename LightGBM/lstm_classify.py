"""
Bi-LSTM — Wi-Fi CSI Multi-task Classification (Zone + Action)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
비교군 모델 (vs MMoE)
  - Per-RX Bi-LSTM 인코더 (가중치 공유)
  - RX Fusion: Concat → Linear(4*hidden*2 → 256)
...
RESULTS_DIR   = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', 'lstm')
...
class BiLSTMModel(nn.Module):
    """
    Per-RX Bi-LSTM → RX Concat Fusion → Dual Task Towers.
    입력: (B, 4, 200, 166)
    """
    ...
    def __init__(self):
        ...
        # RX fusion: 4 × (128*2) = 1024 → 256
        self.fusion = nn.Sequential(
            nn.Linear(NUM_RX * LSTM_HIDDEN * 2, FUSION_DIM),
            nn.GELU(),
            nn.Dropout(0.3),
        )
    ...
    def _encode(self, x):
        """
        x: (B, 4, 200, 166)
        Returns: (B, 4, LSTM_HIDDEN*2)
        """
        B = x.size(0)
        # (B*4, 200, 166)
        seq = x.reshape(B * NUM_RX, L_INPUT, NUM_FEATURES)
        _, (h_n, _) = self.lstm(seq)   # h_n: (layers*2, B*4, hidden)
        # 마지막 레이어의 forward/backward hidden 연결
        h = torch.cat([h_n[-2], h_n[-1]], dim=-1)   # (B*4, hidden*2)
        return h.reshape(B, NUM_RX, LSTM_HIDDEN * 2)

    def forward(self, x):
        B = x.size(0)
        enc   = self._encode(x)                          # (B, 4, 256)
        fused = self.fusion(enc.reshape(B, -1))          # (B, 256)
...
def benchmark_inference(model, loader, device, warmup_batches=3):
    ...
    print(f'\n{"="*60}')
    print('Inference Benchmark  [Bi-LSTM]')
    print(f'{"="*60}')
...
def main():
    ...
    print(f'\n{"="*60}')
    print(f'Training Bi-LSTM  (max {MAX_EPOCHS} epochs, patience={PATIENCE})')
    print(f'{"="*60}')
    ...
    best_epoch   = 0
    patience_cnt = 0
    weight_path  = os.path.join(WEIGHTS_DIR, 'lstm_best.pt')
    ...
    with open(summary_path, 'w') as f:
        ...
        for task, acc in [('zone', zone_acc), ('action', action_acc)]:
            f.write(f'BiLSTM,{task},"{"+".join(TEST_SUBJECTS)}",'
