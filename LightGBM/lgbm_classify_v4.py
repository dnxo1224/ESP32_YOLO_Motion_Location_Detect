...
v1 — LightGBM 기본
  ...
  - 태스크별 특징:
      Zone   : mean + std + min + max + median + IQR + FFT 저주파 + gradient  (2821-dim / RX → 11284-dim)
      Action : mean + std + min + max + median + IQR + FFT 저주파 + gradient  (2821-dim / RX → 11284-dim)
...
def extract_features_window(window):
    """
    (WINDOW_SIZE, 166) → 2821-dim  (Zone / Action 공통)
      ...
      max      : (166,)        — 채널별 최댓값
      median   : (166,)        — 채널별 중앙값
      IQR      : (166,)        — 채널별 75th - 25th 백분위
      FFT 저주파: (K_LOW×166,) — 저주파 빈 파워 (DC 제외, bins 1~K_LOW)
      gradient : (165,)        — 인접 서브캐리어 간 진폭 차이 평균
    """
    mean   = window.mean(axis=0)                                       # (166,)
    std    = window.std(axis=0)                                        # (166,)
    mn     = window.min(axis=0)                                        # (166,)
    mx     = window.max(axis=0)                                        # (166,)
    median = np.median(window, axis=0)                                 # (166,)
    iqr    = (np.percentile(window, 75, axis=0)
              - np.percentile(window, 25, axis=0))                     # (166,)
    ...
    return np.concatenate([mean, std, mn, mx, median, iqr,
                           fft_lowfreq, grad])                         # (2821,)


def extract_zone_features_window(window):
    return extract_features_window(window)
...
def build_windows_from_grids(sample):
    ...
    for start in range(0, WINDOW_SIZE * ((800 - WINDOW_SIZE) // WINDOW_STRIDE + 1),
        ...
        zone_feats.append(np.concatenate(rx_zone))       # (11284,)
...
def plot_feature_importance(clf, task_name):
    """
    LightGBM feature importance를 그룹별로 집계 후 막대그래프 저장.

    특징 그룹 (per RX, 2821-dim):
      mean     :    0 ~  166  (166)
      std      :  166 ~  332  (166)
      min      :  332 ~  498  (166)
      max      :  498 ~  664  (166)
      median   :  664 ~  830  (166)
      IQR      :  830 ~  996  (166)
      FFT      :  996 ~ 2656  (K_LOW×166 = 1660)
      gradient : 2656 ~ 2821  (165)
    4 RX → 11284-dim 전체에서 그룹별 합산.
    """
    # 그룹 정의 (이름, per-RX 시작, per-RX 끝)
    groups = [
        ('mean',     0,    166),
        ('std',      166,  332),
        ('min',      332,  498),
    ...
    importances = clf.feature_importances_   # (11284,)
    group_sums  = {}
    for name, start, end in groups:
        total = 0.0
        for rx in range(4):
            offset = rx * feat_per_rx
            total += importances[offset + start : offset + end].sum()
    ...
    for name, pct in sorted(group_pcts.items(), key=lambda x: -x[1]):
        bar_len = int(pct / 2)
        print(f'    {name:<10} {"█" * bar_len}  {pct:.1f}%')
