...
v1 — LightGBM 기본
  ...
  - 태스크별 특징:
      ...
      Action : mean + std + DFS centroid + DFS spread + gradient  (829-dim / RX → 3316-dim)
...
def extract_zone_features_window(window):
    ...
    return np.concatenate([mean, std, grad])


def extract_action_features_window(window):
    """
    (WINDOW_SIZE, 166) → 829-dim (action 특화)

      mean       : (166,)  — 채널별 평균
      std        : (166,)  — 채널별 표준편차
      DFS centroid: (166,) — 주파수 무게중심 = Σ(f × |F(f)|²) / Σ|F(f)|²  → 움직임 속도
      DFS spread  : (166,) — 주파수 분포 표준편차                           → 움직임 복잡도
    ...
    freqs     = np.arange(fft_power.shape[0], dtype=np.float32)
    power_dc  = fft_power[1:, :]   # DC 제외 → (WINDOW_SIZE//2, 166)
    freqs_dc  = freqs[1:]

    total_power = power_dc.sum(axis=0) + 1e-8  # 0-division 방지
    ...
    return np.concatenate([mean, std, centroid, spread, grad])  # (829,)
...
def build_windows_from_grids(sample):
    ...
    for start in range(0, WINDOW_SIZE * ((800 - WINDOW_SIZE) // WINDOW_STRIDE + 1),
        ...
        action_feats.append(np.concatenate(rx_action))   # (3316,)
