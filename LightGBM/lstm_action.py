"""Bi-LSTM — Action Classification only (단일 태스크 비교군)"""
...
RESULTS_DIR   = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', 'lstm_action')
...
def benchmark_inference(model, loader, device, warmup_batches=3):
    ...
    print(f'\n{"="*60}')
    print(f'Inference Benchmark  [Bi-LSTM / Action]')
    print(f'{"="*60}')
...
def plot_curves(history):
    ...
    plt.suptitle('Bi-LSTM / Action', fontsize=13, y=1.02); plt.tight_layout()
...
def main():
    ...
    device = get_device()
    print(f'Model  : Bi-LSTM / Action only  |  Device: {device}')
    ...
    weight_path = os.path.join(WEIGHTS_DIR, 'lstm_action_best.pt')
    ...
    with open(os.path.join(RESULTS_DIR, 'summary.csv'), 'w') as f:
        ...
        f.write(f'BiLSTM,action,{acc*100:.4f},{best_ep},'
