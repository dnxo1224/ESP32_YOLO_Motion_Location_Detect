"""Transformer — Zone Classification only (단일 태스크 비교군)"""
...
RESULTS_DIR   = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', 'tf_zone')
...
class TransformerZone(nn.Module):
    def __init__(self):
        super().__init__()
...
def benchmark_inference(model, loader, device, warmup_batches=3):
    ...
    print(f'\n{"="*60}')
    print(f'Inference Benchmark  [Transformer / Zone]')
    print(f'{"="*60}')
...
def plot_curves(history):
    ...
    plt.suptitle('Transformer / Zone', fontsize=13, y=1.02); plt.tight_layout()
...
def main():
    ...
    device = get_device()
    print(f'Model  : Transformer / Zone only  |  Device: {device}')
    ...
    model = TransformerZone().to(device)
    ...
    weight_path = os.path.join(WEIGHTS_DIR, 'tf_zone_best.pt')
    ...
    with open(os.path.join(RESULTS_DIR, 'summary.csv'), 'w') as f:
        ...
        f.write(f'Transformer,zone,{acc*100:.4f},{best_ep},'
