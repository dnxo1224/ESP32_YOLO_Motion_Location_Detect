...
from dataset import create_dataloaders, POSITION_NAMES
...
def main():
    ...
    train_loader, val_loader = create_dataloaders(
        DATA_DIR, label_type='position', val_ratio=0.2, batch_size=BATCH_SIZE
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(SAVE_DIR, 'best_position_model.pth'))
    ...
    model.load_state_dict(torch.load(os.path.join(SAVE_DIR, 'best_position_model.pth')))
    _, final_acc, preds, labels = evaluate(model, val_loader, criterion, device)
    
    target_names = [POSITION_NAMES[i] for i in range(NUM_CLASSES)]
